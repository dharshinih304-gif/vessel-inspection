...
export default function LoginPage() {
        ...
        const fetchAndRedirect = async () => {
          try {
            ...
            if (companyData?.company_name) {
              window.location.href = `/company/${companyData.company_name.toLowerCase()}/dashboard`;
            } else {
              window.location.href = `/company/${user.company_id}/dashboard`;
            }
          } catch (e) {
            window.location.href = `/company/${user.company_id}/dashboard`;
          }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalCompanyId = dbUser.company_id;
      // Manually set a mock session in local storage for useAuth fallback
      // Write this BEFORE authenticating to prevent race conditions with onAuthStateChange
      if (typeof window !== 'undefined') {
        localStorage.setItem('user_portal_mode', 'true');
        
        if (dbUser.role?.toUpperCase() === 'SUPERADMIN') {
          // Resolve company ID for the session
      ...
      if (authError) {
        ...
        // We do not throw here because the user's password was already verified against public.users
        // via the verify_user_password RPC above.
      }

      showToast('Login successful! Redirecting...', 'success');

      let targetUrl = '/dashboard';
      let targetCompanyId = finalCompanyId || dbUser.company_id;
      ...
      if (targetCompanyId) {
        try {
          const { data: companyData } = await supabase
            ...
            .single();
          if (companyData?.company_name) {
            targetUrl = `/company/${companyData.company_name.toLowerCase()}/dashboard`;
          } else {
            targetUrl = `/company/${targetCompanyId}/dashboard`;
          }
        } catch (e) {
