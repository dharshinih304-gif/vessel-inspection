...
async function generate() {
  ...
  for (const cat of CATEGORIES) {
    // Delete existing rows for this vessel to ensure 100% no duplicates
    sql += `DELETE FROM ${cat} WHERE vessel_id = '${VESSEL_ID}';\n`;
    ...
    try {
      ...
      console.log(`-- Failed to fetch templates for ${cat}`);
      continue;
    }
    ...
    for (const tmpl of uniqueItems) {
      const s_no = (tmpl.s_no || tmpl.item_no || '').replace(/'/g, "''");
      const rule_ref = (tmpl.rule_ref || '-').replace(/'/g, "''");
      const req = (tmpl.requirements || '-').replace(/'/g, "''");
      const ans = 'GOOD';
      const comments = 'Checked and verified.';
      
      sql += `INSERT INTO ${cat} (vessel_id, company_id, user_id, s_no, rule_ref, requirements, ans, comments, image, report_id) VALUES ('${VESSEL_ID}', '${COMPANY_ID}', '${USER_ID}', '${s_no}', '${rule_ref}', '${req}', '${ans}', '${comments}', '', NULL);\n`;
    }
  }

  sql += 'COMMIT;\n';
  fs.writeFileSync('D:/inspection/frontend/scratch/mass_insert.sql', sql);
